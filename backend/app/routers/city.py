...
def get_city_summary(city_name: str) -> CitySummary:
    sql = f"""
        ...
        WHERE LOWER(city) = LOWER({_q(city_name)})
        LIMIT 1
    """
    try:
...
def get_city_permits(
    ...
    where_parts = [f"LOWER(city) = LOWER({_q(city_name)})"]

    if status is not None:
        where_parts.append(f"status = {_q(status.upper())}")
    if zip_code is not None:
        where_parts.append(f"zip_code = {_q(zip_code)}")

    where_clause = " AND ".join(where_parts)

    count_sql = f"SELECT COUNT(*) FROM {CATALOG}.enriched_permits WHERE {where_clause}"
    data_sql = f"""
        SELECT
            permit_id, zip_code, city, state, system_size_kw, install_date,
            status, latitude, longitude, ac_annual_kwh, electricity_rate_per_kwh,
            annual_savings_usd, co2_offset_metric_tons, enriched_at
        FROM {CATALOG}.enriched_permits
        WHERE {where_clause}
    ...
    permits = [
        Permit(
            permit_id=r[0], zip_code=r[1], city=r[2], state=r[3],
            system_size_kw=r[4], install_date=r[5], status=r[6],
            latitude=r[7], longitude=r[8], ac_annual_kwh=r[9],
            electricity_rate_per_kwh=r[10], annual_savings_usd=r[11],
            co2_offset_metric_tons=r[12], enriched_at=r[13],
        )
        for r in rows
    ]
...
def get_city_forecast(city_name: str) -> CityForecast:
    ...
    monthly_kwh = avg_kwh / 12.0
    monthly_savings = avg_savings / 12.0
    monthly_co2 = avg_co2 / 12.0
    ...
    for i in range(1, 13):
        ...
        proj_kwh.append(round(velocity * monthly_kwh, 2))
        proj_savings.append(round(velocity * monthly_savings, 2))
        proj_co2.append(round(velocity * monthly_co2, 4))
